...
def get_location_point(element):
    if isinstance(element, TextNote):
        ...
        return element.Origin  # This is read-only but can be used to calculate offsets
...
if not selection or len(selection) < 2:
    ...
    # Find the leftmost point among the selected elements
    leftmost_point = None
    for element in selection:
        element_point = get_location_point(element)
        if element_point is not None:
            if leftmost_point is None or element_point.X < leftmost_point.X:
                leftmost_point = element_point

    if leftmost_point is None:
        print("No valid location points found among the selected elements. Do nothing.")
    else:
        # Start a new transaction
        with Transaction(doc, 'Left Align Elements') as t:
            t.Start()

            # Align each selected element to the leftmost point
            for element in selection:
                if isinstance(element, TextNote):
                    element.Coord = XYZ(leftmost_point.X, element.Coord.Y, element.Coord.Z)
                elif isinstance(element, IndependentTag):
                    element.LeaderEnd = XYZ(leftmost_point.X, element.LeaderEnd.Y, element.LeaderEnd.Z)
                elif isinstance(element, SpotDimension):
                    offset_x = leftmost_point.X - get_location_point(element).X
                    ...
                    offset_x = leftmost_point.X - current_box_center.X
                    ...
                    offset = get_location_point(element).X - leftmost_point.X
                    element.Location.Move(XYZ(-offset, 0, 0))
                elif isinstance(element, FamilyInstance) and hasattr(element.Location, 'Point'):
                    offset = element.Location.Point.X - leftmost_point.X
                    element.Location.Move(XYZ(-offset, 0, 0))
                else:
                    # Handle other element types as necessary
