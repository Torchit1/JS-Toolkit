...
if not selection or len(selection) < 2:
    ...
    # Find the lowest point among the selected elements
    lowest_point = None
    for element in selection:
        element_point = get_location_point(element)
        if element_point is not None:
            if lowest_point is None or element_point.Y < lowest_point.Y:
                lowest_point = element_point

    if lowest_point is None:
        print("No valid location points found among the selected elements. Do nothing.")
    else:
        # Start a new transaction
        with Transaction(doc, 'Bottom Align Elements') as t:
            t.Start()

            # Align each selected element to the lowest point
            for element in selection:
                if isinstance(element, TextNote):
                    element.Coord = XYZ(element.Coord.X, lowest_point.Y, element.Coord.Z)
                elif isinstance(element, IndependentTag):
                    element.LeaderEnd = XYZ(element.LeaderEnd.X, lowest_point.Y, element.LeaderEnd.Z)
                elif isinstance(element, SpotDimension):
                    offset_y = lowest_point.Y - get_location_point(element).Y
                    ...
                    offset_y = lowest_point.Y - current_box_center.Y
