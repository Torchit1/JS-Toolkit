...
from Autodesk.Revit.DB import (FilteredElementCollector, Transaction, IndependentTag,
                               Reference, TagMode, TagOrientation, View, ViewType)
...
def tag_elements_in_view(doc, view, elements, tagged_views_info, progress_bar):
    # Collect all existing tags in the view
    existing_tags = FilteredElementCollector(doc, view.Id).OfClass(IndependentTag)

    # Create a set of element ids that are already tagged in the view
    ...
    for idx, element in enumerate(elements):
        ...
        # Check if the element is already tagged in this view
        if element.Id.IntegerValue in already_tagged_element_ids:
            continue

        if not is_element_visible_in_view(doc, view, element):
            continue
        ...
        try:
            IndependentTag.Create(doc, view.Id, Reference(element), False, TagMode.TM_ADDBY_CATEGORY, TagOrientation.Horizontal, center_point)
        except Exception:
            pass
