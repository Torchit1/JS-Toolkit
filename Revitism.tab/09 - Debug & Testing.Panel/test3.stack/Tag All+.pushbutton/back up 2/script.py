    ...
    for idx, element in enumerate(elements):
        ...
        try:
            ...
            if element.Category.Name == "Windows" and view.ViewType == ViewType.FloorPlan:
                ...
                # Set the position of the leader end point
                leader_end_point = XYZ(center_point.X, center_point.Y + 10, center_point.Z)  # Example offset
...
def select_categories(doc):
    ...
    if config.is_shift_click_mode() or EXEC_PARAMS.config_mode:
        category_names = sorted([cat.Name for cat in categories if cat.AllowsBoundParameters])
    else:
        specific_categories = ['Walls', 'Windows', 'Doors', 'Generic Models', 'Furniture']
