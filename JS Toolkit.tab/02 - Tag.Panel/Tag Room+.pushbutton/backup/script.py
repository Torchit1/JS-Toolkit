...
def tag_all_rooms():
    ...
    existing_tag_positions = {}

    with revit.Transaction('Tag All Rooms in All Views'):
        # Collect existing room tag positions
        for view in views:
            existing_tags = DB.FilteredElementCollector(revit.doc, view.Id).OfClass(DB.IndependentTag)
            existing_tag_positions[view.Id] = set()
            for tag in existing_tags:
                if isinstance(tag, DB.IndependentTag) and tag.TagHeadPosition:
                    existing_tag_positions[view.Id].add((tag.TagHeadPosition.X, tag.TagHeadPosition.Y))

        # Create new tags and check for overlap
        for view in views:
            for el in rooms:
                ...
                if isinstance(view, (DB.ViewSection, DB.ViewPlan)):
                    try:
                        ...
                        # Check if the newly created tag overlaps with existing tags
                        new_tag_pos = (room_tag.TagHeadPosition.X, room_tag.TagHeadPosition.Y)
                        if new_tag_pos in existing_tag_positions[view.Id]:
                            # Delete the overlapping tag
                            revit.doc.Delete(room_tag.Id)
